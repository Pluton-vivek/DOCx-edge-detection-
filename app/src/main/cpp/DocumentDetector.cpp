...
vector<vector<cv::Point>> DocumentDetector::scanPoint(Mat &edged, Mat &image, bool drawContours)
{
    ...
    // --- Optimized Single-Pass Edge Pipeline ---
    cv::Mat gray;
    if (temp1.channels() == 4) {
        cv::cvtColor(temp1, gray, cv::COLOR_BGRA2GRAY);
    } else if (temp1.channels() == 3) {
        cv::cvtColor(temp1, gray, cv::COLOR_BGR2GRAY);
    } else {
        cv::extractChannel(temp1, gray, 0);
    }
    
    if (gray.type() != CV_8UC1 && gray.type() != CV_16UC1) {
        gray.convertTo(gray, CV_8UC1);
    }

    // Fast CLAHE with larger tile size (16x16)
    ...
    // Auto-threshold via Otsu to dynamically scale Canny bounds
    cv::Mat dummyOtsu;
    double otsuThresh = cv::threshold(gray, dummyOtsu, 0, 255, cv::THRESH_BINARY | cv::THRESH_OTSU);

    // Dynamic Canny bounds based on Otsu (standard 1:2 ratio)
    double cannyLow  = std::max(5.0, std::min(otsuThresh * 0.33, 240.0));
    double cannyHigh = std::max(10.0, std::min(otsuThresh * 0.66, 250.0));

    cv::Canny(gray, edged, cannyLow, cannyHigh);
